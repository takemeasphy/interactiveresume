...
function setLanguage(lang) {
  ...
  const openPlanet = planets.find((p) =>
    p.classList.contains("planet--info-open")
  );
  if (openPlanet) {
...
function openInfoForPlanet(planet) {
  const titleUk = planet.dataset.titleUk || planet.dataset.title || "";
  const titleEn = planet.dataset.titleEn || titleUk;
  const title = currentLang === "uk" ? titleUk : titleEn;

  const selector = `.planet-info-content[data-lang="${currentLang}"]`;
  const contentNode = planet.querySelector(selector);
  ...
  const panelWidth = infoPanel.offsetWidth || 340;

  let left = planetRect.left - skyRect.left + planetRect.width + 24;
  let top = planetRect.top - skyRect.top + planetRect.height / 2;

  if (left + panelWidth > skyRect.width - 16) {
    left = planetRect.left - skyRect.left - panelWidth - 24;
    if (left < 16) left = 16;
  }
